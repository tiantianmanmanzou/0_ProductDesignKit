...
根据提供的项目背景内容，按照工作流程和样例要求，输出一份详细的产品功能设计说明文档。
要求：
- 要严格遵守设计原则与规范。
- 不要遗漏页面或者弹窗的示意图和功能说明。
...
## 产品流程设计原则 ( Process Design Principles)
...
## 页面内功能设计原则 (In-Page Function Design Principles)
...
**输出要求：** 在进入下一步前，必须明确说明已理解并将在后续设计中严格遵循的设计原则。
...
### 功能说明
#### 页面说明
1. 页面布局图：清晰地绘制出页面布局，要求：
  - 使用ASCII字符清晰地绘制。
  - 有多个tab时，请分别给每个tab的布局设计和交互说明。
  - 不要新增不必要的元素。
  - 不要遗漏元素。
2. 元素交互说明：创建一个Markdown表格，通过元素的文本标签来详细解释其交互细节。表格必须包含以下列，并提供清晰描述：
  - 元素名称: 元素的文本标签。
...
3. 各功能区详情说明：对各个功能区具体功能说明，按照以下原则执行:
  - 按照上述"通用元素描述规范"对列表、按钮、表单等进行详细描述
  - 所有功能区的设计都应围绕核心列表展开，明确与列表的关系
...
#### 弹窗说明
1. 弹窗布局图：清晰地绘制出弹窗布局，要求：
  - 使用ASCII字符清晰地绘制。
  - 页面中每个弹窗的相关说明都要绘制，不要遗漏。
  - 不要新增不必要的元素。
  - 不要遗漏元素。
...
**要求：**
为产品中的每个页面和弹窗，生成高保真度的ASCII线框图和交互说明。设计原则与方法论：
- 尼尔森十大可用性原则 (Nielsen's Heuristics)
...
- 表头字段:| 字段名称 | 数据类型 | 枚举值/说明            | 是否支持搜索(列头筛选) | 默认显隐 | 排序   |
  | -------- | -------- | ---------------------- | ---------------------- | -------- | ------ |
  | 用户名   | 字符串   |                        | 是                     | 显示     | 支持   |
  | 姓名     | 字符串   |                        | 是                     | 显示     | 支持   |
  | 邮箱     | 字符串   |                        | 是                     | 显示     | 支持   |
...
- 操作项 (Row-level Actions):| 名称     | 触发条件                               | 交互行为                                 | 主要作用         | 权限控制                   |
  | -------- | -------------------------------------- | ---------------------------------------- | ---------------- | -------------------------- |
...
- 弹窗标题: "新增用户"
- 触发机制: 点击页面级"新增用户"按钮。
- 表单字段:| 字段标签 | UI控件类型        | 是否必填 | 校验规则                     | 默认值/来源 | 提示信息/备注                       |
  | :------- | :---------------- | :------- | :--------------------------- | :---------- | :---------------------------------- |
