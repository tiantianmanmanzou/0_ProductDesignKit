...
- **页面布局图**：ASCII绘制完整页面，开头和结尾要加上"```"符号，便于在markdown中显示。
    - **字段显示要求**：对于功能调整页面，图中只需明确展示本次**新增或调整**的字段。所有已有未变更的字段统一表示为 `已有字段`，其值显示为 `....`。
- **交互说明**：操作元素的触发和响应
...
- 数据列表：包含字段名、数据类型、是否必填、校验规则、默认值、是否可编辑、数据来源、显示优先级、排序支持等字段。**对于功能调整页面，此部分仅需描述本次新增或调整的字段，无需罗列已有字段。**
