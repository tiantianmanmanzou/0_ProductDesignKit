...
以结构树形式，清晰列出在 1.1 功能模块拆解中定义的所有后端功能模块，包括后端服务、API、测试相关所有内容。
...
以树状结构或列表形式，清晰列出在 1.1 功能模块拆解中定义的所有前端组件，包括页面、组件、E2E集成测试组件等所有内容。
...
*   **代码结构规划:**
    *   **目标:** 清晰展示本模块在整个后端项目中的文件和目录位置。
    *   **要求:**
        *   基于 **[项目总体设计文档]** 中规划的后端结构树，仅抽取出与 **当前模块** 相关的所有文件和目录（包括功能代码和测试文件）。
        *   如果总体设计有遗漏或不合理之处，可在此处进行微调和补充，确保设计的完整性。
    *   **结构树 (示例):**
        ```tree
        ├── app/
        │   ├── models/
...
*   **代码结构规划:**
    *   **目标:** 清晰展示本模块在整个前端项目中的文件和目录位置。
    *   **要求:**
        *   基于 **[项目总体设计文档]** 中规划的前端结构树，仅抽取出与 **当前模块** 相关的所有文件和目录（包括页面、组件、状态管理和测试文件）。
        *   如果总体设计有遗漏或不合理之处，可在此处进行微调和补充，确保设计的完整性。
    *   **结构树 (示例):**
        ```tree
        ├── src/
...
*   **本模块测试文件结构:**
    *   **目标:** 集中展示本模块所有相关的测试文件，以便测试人员和开发人员快速定位。
    *   **要求:**
        *   基于 **[第二部分后端详细设计方案和前端详细设计方案]** 中规划的完整结构树，分别抽取出与 **当前模块** 相关的 **所有层级** 的测试文件（后端单元测试、后端集成测试、前端组件测试、E2E集成测试等）。
    *   **结构树 (示例):**
