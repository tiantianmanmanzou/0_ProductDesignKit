...
- **枚举值说明要单独成列**：字段为枚举/选择类控件时，在字段字典的`枚举值说明`列按“枚举值：含义业务作用”逐条说明；非枚举填`-`（动态枚举需注明来源与取值含义）
...
其中枚举/选择类字段必须填写字段字典的`枚举值说明`列，用于解释每个枚举值的含义与业务作用；非枚举填`-`。
...
| 查询筛选区 | 状态 / status | 枚举 / select | 非必填 | 全部 | 全部：不按状态过滤；<br>启用：仅展示启用用户；<br>禁用：仅展示禁用用户 | 固定枚举 |
| 查询筛选区 | 角色 / roleId | 枚举 / select | 非必填 | 全部 | 全部：不按角色过滤；<br>`roleId`：角色ID（来自角色管理模块API；用于按角色筛选用户） | 角色管理模块API |
| 列表区 | 用户名 / username | 文本 / text | 必显；3-20字符（创建校验） | - | - | 唯一性校验 |
| 列表区 | 邮箱 / email | 邮箱 / text | 必显；邮箱格式（创建校验） | - | - | 唯一性校验 |
| 列表区 | 状态 / status | 枚举 / tag | 必显 | - | 启用：账号可登录/可用；<br>禁用：账号不可登录/冻结 | 影响登录权限 |
...
| 表单区 | 部门 / department | 字符串 / select | 非必填 | 空 | `<departmentId>`：部门ID（来自部门列表API；业务作用：标识用户所属部门） | API动态获取 |
| 表单区 | 角色 / roles | 数组 / checkbox | 必填；至少选择1个角色 | 普通用户 | 管理员：拥有管理权限；<br>普通用户：基础使用权限；<br>测试员：测试用途权限 | 角色管理模块API |
| 表单区 | 状态 / status | 枚举 / radio button | 必填 | 启用 | 启用：账号可登录/可用；<br>禁用：账号不可登录/冻结 | 固定列表 |
