...
###### **页面主体功能说明**
    ...
    - **重点强调**：对于有业务对象的页面，必须明确说明列表在页面中的核心地位，以及其他功能如何围绕列表展开。

###### **页面布局设计**

    页面布局设计需要清晰区分主体页面和弹窗设计，避免混合在一起。按照以下结构输出：

    **主体页面布局（列表为中心）**
    - **列表主体原则**：对于有业务对象的页面，列表必须作为页面的核心主体，占据页面的主要区域
    - **功能层级划分**：
      - 页面级操作（新增、导入、批量处理等）位于列表上方
      - 查询筛选区域位于列表上方，为列表数据提供过滤功能
      - 行级操作（编辑、删除、详情等）位于列表内部每行的操作列
      - 分页控制位于列表下方
    - **ASCII绘制要求**：
      - 使用ASCII字符绘制主体页面的示意图
      - 示意图必须突出列表的主体地位，列表区域应占用页面主要空间
      - 清晰标识页面级、查询级、行级三个层次的功能区域
      - 展示列表与其他功能区域的关系和交互流程

    **弹窗设计（围绕列表对象）**
    - **弹窗设计原则**：每个弹窗都要单独设计，不与主体页面混合
    - **业务对象导向**：所有弹窗设计都应围绕列表中的业务对象展开，包括：
      - 对象的创建（新增弹窗）
      - 对象的修改（编辑弹窗）
      - 对象的查看（详情弹窗）
      - 对象的状态变更（确认弹窗）
      - 对象的批量处理（批量操作弹窗）
    - **弹窗分类设计**：对于涉及弹窗的功能，必须为每种弹窗类型提供独立的ASCII示意图，包括但不限于：
      - **新增弹窗**：展示表单字段的完整布局，包含实时反馈（进度条、状态提示）
      - **编辑弹窗**：展示字段的可编辑状态和约束，明确哪些字段不可编辑
      - **详情弹窗**：展示只读信息的布局方式，支持查看完整信息
      - **预览弹窗**：展示文件或数据预览，保持原始格式
      - **确认弹窗**：展示操作确认的交互设计
      - **任务详情弹窗**：展示任务执行的详细信息和日志
      - **多模式弹窗**：如果弹窗支持多种模式（如可视化/SQL配置），需要分别展示每种模式的布局
    - **弹窗命名规范**：每个弹窗都要有明确的标题，如"弹窗1：项目创建弹窗"、"弹窗2：Excel预览弹窗"
    - **弹窗功能完整性**：每个弹窗的设计要包含完整的交互流程，包括打开、操作、关闭
    - **列表联动机制**：明确说明弹窗操作完成后如何更新主页面列表的状态
...
- **列表为中心的交互模式**：
  ...
  - 通过查询筛选功能在列表中定位具体对象
  - 通过行级操作直接操作列表中的业务对象
  - 通过页面级操作创建新的业务对象或对列表进行批量处理
- **数据流转闭环**：
  - 主体页面交互流程：用户在主页面的操作路径，以列表浏览为起点
  - 弹窗交互流程：各个弹窗的操作流程，以及与列表的关联
  - 数据更新机制：操作完成后的数据更新和列表刷新机制
  - 状态同步：确保列表状态与业务对象状态的实时同步
...
**弹窗1：新增资产弹窗**
  ...
  |  新增资产                                         [×]   |
  +----------------------------------------------------------+
  | IP地址: [192.168.1.1___] * 必填   资产名称: [Web服务器______] * 必填 |
  | 端口: [80___]                     资产类型: [主机 ▼] * 必填 |
  | 所属应用: [电商系统 ▼] * 必填     所属部门: [技术部 ▼] * 必填 |
  | 负责人: [张三______] * 必填       联系方式: [13800138000____] * 必填 |
  | 环境类型: (●) 生产  ( ) 测试  ( ) 开发 * 必填           |
  | 备注: [___________________________________________]     |
  +----------------------------------------------------------+
  | 上传进度: [████████████████████] 100%               |
  | 验证状态: IP地址验证通过，资产名称唯一性检查中...       |
  +----------------------------------------------------------+
  |                 [取消] [确认]                          |
  +----------------------------------------------------------+
...
**弹窗2：编辑资产弹窗**
  ...
  |  编辑资产                                         [×]   |
  +----------------------------------------------------------+
  | 资产ID: [A001] (不可编辑)   IP地址: [192.168.1.1] (不可编辑) |
  | 资产名称: [Web服务器______]  端口: [80___]              |
  | 资产类型: [主机 ▼] (仅管理员)  所属应用: [电商系统 ▼]    |
  | 所属部门: [技术部 ▼] (条件可编辑)  负责人: [张三______]   |
  | 联系方式: [13800138000____]  环境类型: (●) 生产  ( ) 测试 |
...
**弹窗3：资产详情弹窗**
  ...
  +----------------------------------------------------------+
  | 基本信息：                                              |
  | • 资产ID：A001                 • IP地址：192.168.1.1    |
  | • 资产名称：Web服务器          • 端口：80              |
  | • 资产类型：主机               • 环境类型：生产          |
  +----------------------------------------------------------+
  | 归属信息：                                              |
  | • 所属应用：电商系统           • 所属部门：技术部        |
  | • 负责人：张三                 • 联系方式：13800138000  |
  +----------------------------------------------------------+
  | 系统信息：                                              |
  | • 创建时间：2024-01-01 10:00   • 最后更新：2024-01-15   |
  | • 认领状态：已认领             • 资产状态：正常          |
  +----------------------------------------------------------+
  | 备注：该服务器用于处理电商平台的核心业务逻辑...           |
  +----------------------------------------------------------+
  |                    [关闭]                              |
  +----------------------------------------------------------+
...
| 导出     | 工具性操作：将当前列表中的数据导出为文件。   | 列表区域有数据时启用；用户具有导出权限。    | 点击后，触发文件下载流程。支持导出当前筛选条件下的所有数据为Excel格式文件，文件名格式为"CMDB平台资产查询_YYYYMMDD_HHMMSS.xlsx"。导出内容包含列表中所有可见字段的数据。  | 次按钮   | 无       |
...
- 表头字段:
  ...
  | 标准系统    | 字符串         | 操作系统名称            | 否                     | 显示     | 支持 |
  | 资产状态    | 字符串         | (需定义具体状态枚举值)  | 否                     | 显示     | 支持 |
  | 数据库实例名 | 字符串         |                         | 否                     | 显示     | 支持 |
  | 业务名称    | 字符串         |                         | 是                     | 显示     | 不支持 |
  | 系统管理员   | 字符串         |                         | 否                     | 显示     | 支持 |
  | 创建时间    | 日期时间       |                         | 是                     | 显示     | 不支持 |
...
- 操作项 (Row-level Actions):
  ...
  | ---- | -------------------------------------- | ---------------------------------------- | ---------------- | ------------------------ |
  | 编辑 | 资产状态为"已认领"且用户有编辑权限时显示 | 点击后，弹出"编辑资产"弹窗。             | 修改资产信息     | 用户具有"资产编辑"权限 |
...
- 表单字段:
  ...
  | :--------- | :--------------------- | :------- | :--------- | :------------------------- | :----------------- | :------------------------------------------------- |
  | IP地址     | 输入框                 | 是       | 是         | 有效IP格式                 | 从列表带入         | 资产的唯一标识之一                                 |
  | 资产名称   | 输入框                 | 是       | 是         | 不能为空，长度限制 (e.g. 2-50) | 从列表带入         |                                                    |
  | 端口       | 输入框 (数字)          | 否       | 是         | 0-65535                    | 从列表带入         |                                                    |
  | 资产类型   | 下拉选择器             | 是       | 是         | 无                         | 从列表带入         | 选项：主机/数据库/中间件/其他                      |
  | 所属应用   | 选择器 (或输入框)      | 是       | 是         | 无                         | 用户填写           | 用户填写的资产所属的业务应用系统                   |
  | 所属部门   | 选择器 (或输入框)      | 是       | 是         | 无                         | 用户填写           | 用户填写的资产所属的部门                           |
  | 负责人     | 输入框                 | 是       | 是         | 无                         | 用户填写           |                                                    |
  | 联系方式   | 输入框                 | 是       | 是         | 手机号/邮箱格式 (可选其一) | 用户填写           |                                                    |
  | 环境类型   | 下拉选择器             | 是       | 是         | 无                         | 用户选择           | 选项：生产、测试、开发等 (根据实际业务定义)        |
  | 备注       | 文本域 (TextArea)      | 否       | 是         | 长度限制 (e.g. 0-200)      | 用户填写           |                                                    |
- 操作按钮:
  | 按钮文案 | 主要作用                     | 交互行为                                                                                               |
  | -------- | ---------------------------- | ------------------------------------------------------------------------------------------------------ |
...
- 表单字段:
  | 字段标签   | UI控件类型             | 是否必填 | 是否可编辑 | 校验规则                   | 默认值/来源        | 提示信息/备注                                      |
  | :--------- | :--------------------- | :------- | :--------- | :------------------------- | :----------------- | :------------------------------------------------- |
  | 资产ID     | 文本展示               | N/A      | 否         | 无                         | 从列表带入         | 系统生成的唯一标识，不可修改                        |
  | IP地址     | 输入框                 | 是       | 否         | 有效IP格式                 | 从列表带入         | 资产的唯一标识，创建后不可修改                      |
  | 资产名称   | 输入框                 | 是       | 是         | 不能为空，长度限制 (e.g. 2-50) | 从列表带入         | 可自由编辑                                         |
  | 端口       | 输入框 (数字)          | 否       | 是         | 0-65535                    | 从列表带入         | 可自由编辑                                         |
  | 资产类型   | 下拉选择器             | 是       | 条件可编辑  | 无                         | 从列表带入         | 仅管理员权限可编辑，普通用户只读                    |
  | 所属应用   | 选择器 (或输入框)      | 是       | 是         | 无                         | 从列表带入         | 可自由编辑                                         |
  | 所属部门   | 选择器 (或输入框)      | 是       | 条件可编辑  | 无                         | 从列表带入         | 仅本部门用户和上级部门用户可编辑                    |
  | 负责人     | 输入框                 | 是       | 是         | 无                         | 从列表带入         | 可自由编辑                                         |
  | 联系方式   | 输入框                 | 是       | 是         | 手机号/邮箱格式 (可选其一) | 从列表带入         | 可自由编辑                                         |
  | 环境类型   | 下拉选择器             | 是       | 是         | 无                         | 从列表带入         | 选项：生产、测试、开发等                           |
  | 创建时间   | 文本展示               | N/A      | 否         | 无                         | 从列表带入         | 系统字段，不可修改                                 |
  | 备注       | 文本域 (TextArea)      | 否       | 是         | 长度限制 (e.g. 0-200)      | 从列表带入         | 可自由编辑                                         |
...
- 操作按钮:
  | 按钮文案 | 主要作用                     | 交互行为                                                                                               |
  | -------- | ---------------------------- | ------------------------------------------------------------------------------------------------------ |
  | 确认     | 保存修改后的资产信息         | 校验表单。通过后，提交数据到后端用于更新资产。成功后关闭弹窗，并刷新列表。若失败，则在弹窗内提示错误信息。 |
  | 取消     | 关闭弹窗，不保存任何更改     | 点击后直接关闭弹窗。                                                                                   |
...
- 表单字段 :
  ...
  | :--------- | :--------------------- | :--------- | :---------- | :------------------------------------------------- |
  | IP地址     | 文本展示               | 否         | 从列表带入  | 资产的唯一标识之一                                 |
  | 资产名称   | 文本展示               | 否         | 从列表带入  |                                                    |
  | 端口       | 文本展示               | 否         | 从列表带入  |                                                    |
  | 资产类型   | 文本展示               | 否         | 从列表带入  | 选项：主机/数据库/中间件/其他                      |
  | 所属应用   | 文本展示               | 否         | 从列表带入  | 用户填写的资产所属的业务应用系统                   |
  | 所属部门   | 文本展示               | 否         | 从列表带入  | 用户填写的资产所属的部门                           |
  | 负责人     | 文本展示               | 否         | 从列表带入  |                                                    |
  | 联系方式   | 文本展示               | 否         | 从列表带入  |                                                    |
  | 环境类型   | 文本展示               | 否         | 从列表带入  | 选项：生产、测试、开发等 (根据实际业务定义)        |
  | 备注       | 文本展示 (或文本域只读) | 否         | 从列表带入  |                                                    |
- 操作按钮:
  | 按钮文案 | 主要作用                     | 交互行为             |
  | -------- | ---------------------------- | -------------------- |
...
- 功能描述: 将当前列表中符合筛选条件的数据导出为Excel文件，便于用户进行离线分析和数据备份。
- 触发机制: 点击页面级"导出"按钮。
- 导出范围:
  - 数据范围：导出当前筛选条件下的所有数据（不受分页限制）
  - 字段范围：包含列表中所有可见字段的数据
- 文件格式与命名:
  | 属性     | 规范                                    | 说明                           |
  | -------- | --------------------------------------- | ------------------------------ |
  | 文件格式 | Excel (.xlsx)                           | 标准Excel格式，便于用户查看编辑 |
  | 文件名   | CMDB平台资产查询_YYYYMMDD_HHMMSS.xlsx   | 包含功能名称和导出时间戳       |
...
- 权限控制:
  - 用户需具有"数据导出"权限才能看到并使用导出按钮
  - 导出操作会记录到操作日志中，包含用户信息、导出时间、导出条件等
- 性能考虑:
  - 当数据量超过10000条时，提示用户数据量较大，建议缩小筛选范围
...
- 异常处理:
  ...
  | 权限不足         | "您没有数据导出权限，请联系管理员"       | 联系管理员申请相应权限         |

#### 页面交互流程

**主体页面交互流程**
1. 用户进入CMDB平台资产查询页面，查看资产列表
2. 使用筛选条件进行数据查询，系统实时更新列表
3. 点击"新增"按钮，打开新增资产弹窗
4. 点击"导出"按钮，下载当前筛选条件下的数据
5. 在列表中选择资产，执行详情、编辑、认领等操作

**弹窗交互流程**
1. **新增资产弹窗**：填写资产信息 → 实时验证 → 提交数据 → 关闭弹窗 → 刷新列表
2. **编辑资产弹窗**：预填充数据 → 修改可编辑字段 → 保存更改 → 关闭弹窗 → 更新列表
3. **详情弹窗**：展示完整资产信息 → 支持查看各类详情 → 关闭弹窗

**数据流转**
1. 新增/编辑操作完成后，自动刷新主页面列表
2. 系统实时更新资产状态和相关信息
3. 所有操作记录到操作日志中，支持审计追踪
